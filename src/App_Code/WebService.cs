...
public class WebService : System.Web.Services.WebService
{
    ...
    public string GetStartTable()
    {
        ...
        try
        {
            using (HostingEnvironment.Impersonate())
            using (SqlConnection db = this.OpenDatabase())
            using (SqlCommand cmd = new SqlCommand("SELECT rr.ID_RACE_REGISTRATION_RESULT, rr.START_NUMBER, rr.PRESENTED, rr.START_TIME, rr.FINISH_TIME, cm.FIRST_NAME, cm.LAST_NAME, cm.BIRTH_DATE, c.CODE, c.NAME as CATEGORY, r.NAME as RACE, rr.TEAM, cm.ADDRESS, a.REGISTRATION_END, rr.REGISTRATION_DATE FROM RaceRegistrationResults rr, RaceCompetitors cm, RaceCategory c, Races r, RaceActions a WHERE cm.ID_RACE_COMPETITOR = rr.ID_RACE_COMPETITOR AND c.ID_RACE_CATEGORY = rr.ID_RACE_CATEGORY AND c.ID_RACE = r.ID_RACE AND a.ID_RACE_ACTION = r.ID_RACE_ACTION AND a.ID_RACE_ACTION = 5 ORDER BY rr.START_TIME ASC, rr.START_NUMBER ASC", db))
            {
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    int registered = 0;
    ...
    public string GetFinishTable()
    {
        ...
        try
        {
            ...
            using (HostingEnvironment.Impersonate())
            using (SqlConnection db = this.OpenDatabase())
            using (SqlCommand cmd = new SqlCommand("SELECT rr.ID_RACE_REGISTRATION_RESULT, rr.START_NUMBER, rr.FINISH_TIME, cm.FIRST_NAME, cm.LAST_NAME, cm.BIRTH_DATE, c.CODE, c.NAME as CATEGORY, r.NAME as RACE, rr.TEAM, cm.ADDRESS, (rr.FINISH_TIME - rr.START_TIME) as RACE_TIME FROM RaceRegistrationResults rr, RaceCompetitors cm, RaceCategory c, Races r, RaceActions a WHERE cm.ID_RACE_COMPETITOR = rr.ID_RACE_COMPETITOR AND c.ID_RACE_CATEGORY = rr.ID_RACE_CATEGORY AND c.ID_RACE = r.ID_RACE AND a.ID_RACE_ACTION = r.ID_RACE_ACTION AND a.ID_RACE_ACTION = 5 AND rr.FINISH_TIME is not null ORDER BY r.ID_RACE ASC, RACE_TIME ASC, rr.GROUP_ORDER ASC", db))
            {
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    int order = 0;
