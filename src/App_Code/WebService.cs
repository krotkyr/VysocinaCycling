...
public class WebService : System.Web.Services.WebService
{
            ...
            using (SqlCommand cmd = new SqlCommand("SELECT rr.ID_RACE_REGISTRATION_RESULT, rr.START_NUMBER, rr.PRESENTED, rr.FINISH_TIME, cm.FIRST_NAME, cm.LAST_NAME, cm.BIRTH_DATE, c.CODE, c.NAME as CATEGORY, r.NAME as RACE, rr.TEAM, cm.ADDRESS, a.REGISTRATION_END, rr.REGISTRATION_DATE, rr.DISCOUNTED_FEE  FROM RaceRegistrationResults rr, RaceCompetitors cm, RaceCategory c, Races r, RaceActions a WHERE cm.ID_RACE_COMPETITOR = rr.ID_RACE_COMPETITOR AND c.ID_RACE_CATEGORY = rr.ID_RACE_CATEGORY AND c.ID_RACE = r.ID_RACE AND a.ID_RACE_ACTION = r.ID_RACE_ACTION AND a.ID_RACE_ACTION = 6 ORDER BY c.ID_RACE_CATEGORY ASC, c.NAME DESC, rr.START_NUMBER ASC, rr.REGISTRATION_DATE ASC", db))
            {
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        registrationstable += "<td class='notPrintable'" + ((bool)reader["PRESENTED"] ? (reader["FINISH_TIME"].ToString() == "" ? (" style=\"color:#008000\">Presentov·n") : (" style=\"color:#0066CC\">V cÌli")) : (((DateTime)reader["REGISTRATION_END"] > (DateTime)reader["REGISTRATION_DATE"]) ? (reader["DISCOUNTED_FEE"].ToString() == "" ? ">P¯ihl·öen" : ">Zaplaceno") : (" style=\"color:#FF3300\">Po termÌnu"))) + "</td>";
    ...
    public string SendSilnicniZavodEmail(string jmeno, string prijmeni, string narozeni, string kategorie, string klub, string licence, string mesto, int platba, int fee, string emailadress, string telefon, string skola, string popis, string typzadosti, bool neposlatemail)
    {
        ...
        {
            string startFee = fee.ToString() + " KË";
            if (platba == 2 && fee > 300)
            {
                startFee = (fee - 50).ToString() + " KË";
            }
            string body = "JmÈno: " + jmeno + '\n' + "P¯ÌjmenÌ: " + prijmeni + '\n' + "Datum narozenÌ: " + narozeni + '\n' + "Kategorie: " + kategorie + '\n' + "Klub: " + klub + '\n' + "»Ìslo a kÛd licence: " + licence + '\n' + "MÏsto: " + mesto + '\n' + "Platba: " + ((platba == 1) ? "V kancel·¯i z·vodu" : "P¯evodem na ˙Ëet") + '\n' + "StartovnÈ: " + startFee + '\n' + "Email: " + emailadress + '\n' + "Telefon: " + telefon;
            if (platba == 2)
            {
                body = body + '\n' + '\n' + "Platbu prosÌm zaölete na ˙Ëet 2500260342/2010 nejpozdÏji do 30.6.2020." + '\n' + "Do pozn·mky uveÔte text DukovanskÈ okruhy 2020 a jmÈno z·vodnÌka, za kterÈho platÌte." + '\n' + "Jako variabilnÌ simbol uveÔte datum narozenÌ z·vodnÌka ve form·tu 'ddmmrrrr'." + '\n' + "Jakmile bude platba ovÏ¯ena zobrazÌ se ve startovnÌ listinÏ u z·vodnÌka stav Zaplaceno.";
            }
