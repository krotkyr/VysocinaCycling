...
public class WebService : System.Web.Services.WebService
{
    ...
    public string GetRegistrationsTable()
    {
        ...
        try
        {
            using (HostingEnvironment.Impersonate())
            using (SqlConnection db = this.OpenDatabase())
            using (SqlCommand cmd = new SqlCommand("SELECT rr.ID_RACE_REGISTRATION_RESULT, rr.START_NUMBER, rr.PRESENTED, rr.FINISH_TIME, cm.FIRST_NAME, cm.LAST_NAME, cm.BIRTH_DATE, c.CODE, c.NAME as CATEGORY, r.NAME as RACE, rr.TEAM, cm.ADDRESS, a.REGISTRATION_END, rr.REGISTRATION_DATE FROM RaceRegistrationResults rr, RaceCompetitors cm, RaceCategory c, Races r, RaceActions a WHERE cm.ID_RACE_COMPETITOR = rr.ID_RACE_COMPETITOR AND c.ID_RACE_CATEGORY = rr.ID_RACE_CATEGORY AND c.ID_RACE = r.ID_RACE AND a.ID_RACE_ACTION = r.ID_RACE_ACTION AND a.ID_RACE_ACTION = 3 ORDER BY c.AGE_FROM DESC, c.AGE_TO ASC, c.NAME DESC, rr.START_NUMBER ASC, rr.REGISTRATION_DATE ASC", db))
            {
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    int registered = 0;
    ...
    public string GetFinishTable()
    {
        ...
        try
        {
            using (HostingEnvironment.Impersonate())
            using (SqlConnection db = this.OpenDatabase())
            using (SqlCommand cmd = new SqlCommand("SELECT rr.ID_RACE_REGISTRATION_RESULT, rr.START_NUMBER, rr.FINISH_TIME, cm.FIRST_NAME, cm.LAST_NAME, cm.BIRTH_DATE, c.CODE, c.NAME as CATEGORY, r.NAME as RACE, rr.TEAM, cm.ADDRESS, (rr.FINISH_TIME - r.START_TIME) as RACE_TIME FROM RaceRegistrationResults rr, RaceCompetitors cm, RaceCategory c, Races r, RaceActions a WHERE cm.ID_RACE_COMPETITOR = rr.ID_RACE_COMPETITOR AND c.ID_RACE_CATEGORY = rr.ID_RACE_CATEGORY AND c.ID_RACE = r.ID_RACE AND a.ID_RACE_ACTION = r.ID_RACE_ACTION AND a.ID_RACE_ACTION = 3 AND rr.FINISH_TIME is not null ORDER BY c.AGE_FROM DESC, c.AGE_TO ASC, RACE_TIME ASC, rr.GROUP_ORDER ASC", db))
            {
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    int order = 0;
                    ...
                    while (reader.Read())
                    {
                        if (reader["CATEGORY"].ToString() != category)
                        {
                            ...
                            finishtable += "<td class='notPrintable'><input id=\"finished" + reader["CODE"] + "\" type=\"button\" runat=\"server\" value=\"Tisk v˝sledk˘\" class=\"printCategoryFinished userRoleEditor\" style=\"height:30px; width:100px; font-size:8px; padding:1px; display:none;\" /></td></tr></table>";
                            finishtable += "<table>";
                            finishtable += "<tr style='font-weight: bold; font-style: italic; font-size: small;'>";
                            finishtable += "<td width='30px'>Po¯.</td>";
                            finishtable += "<td width='30px'>St.Ë.</td>";
                            finishtable += "<td width='80px'>JmÈno</td>";
                            finishtable += "<td width='80px'>P¯ÌjmenÌ</td>";
                            finishtable += "<td width='60px'>RoËnÌk</td>";
                            finishtable += "<td width='180px'>T˝m</td>";
                            finishtable += "<td width='80px'>»as</td>";
                            finishtable += "<td width='80px'>Ztr·ta</td>";
                            finishtable += "</tr>";
                            order = 0;
        ...
        if (LoggedUser != null)
        {
            if (MyNewMembershipProvider.ValidateUser(LoggedUser.UserName, heslo))
            {
                ...
                if (MyNewMembershipProvider.DeleteUser(user, true))
                {
                    ...
                    return "Uûivatel byl smaz·n." + SendEmail(subject, body, "info@vysocinacycling.cz", emailadress);
                }
                else
